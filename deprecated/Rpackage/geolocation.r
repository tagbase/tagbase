...
# Purpose: Light based geolocation from tagbase

library(RODBC)
##################### start get.trackit.dat #################################
# Author: Tim Sippel
# Purpose: extract light data (and eventually SST too) from tagbase and put in correct format for function prepit(trackit)
# db.object is object created by dbCon() 
# TagID is PTT number of a tag, (example TagID="2994") # change to desired PTT number
get.trackit.dat<-function(db.object, TagID){
  ll.table<-sqlFetch(channel=db.object, "Proc_WC_LocationsLight")
  ll.PSAT<-ll.table[ll.table$TagID == TagID,]
  ll.PSAT<-ll.PSAT[order(ll.PSAT$TimeDate),]
  track<-data.frame(year=as.numeric(substr(ll.PSAT$TimeDate,1,4)), month=as.numeric(substr(ll.PSAT$TimeDate,6,7)), day=as.numeric(substr(ll.PSAT$TimeDate,9,10)),
    hour=as.numeric(substr(ll.PSAT$TimeDate,12,13)), min=as.numeric(substr(ll.PSAT$TimeDate,15,16)), sec=as.numeric(substr(ll.PSAT$TimeDate,18,19)), 
    depth=as.numeric(ll.PSAT$depth), light=as.numeric(ll.PSAT$light), temp=0)
  }
##################### end get.trackit.dat #################################
...
##################### start fit2csv #######################################
# Author: Tim Lam
# Purpose: to export trackit results (input/output parameters and track estimate) to three .csv files
# fit is object created by running trackit()
# folder is character vector to directory pathway to store outputs (ie. "/.../Tagbase/analysis/trackit_outputs")
# name is the name given to three output files (with extensions a,b,c) (ie. name="2994")
